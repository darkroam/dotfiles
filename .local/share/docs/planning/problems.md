...
- **证据/证据等级**：
  ...
  - **用户回忆**：用户 2026-09-03 本侧 `deep` 合盖实测确认通过，4 种组合（拔电/接电 × 外屏/无外屏）
    全部确认通过；验收组合与日志未入档，补交后可升为「实测」。
...
- **解决**：规避已不再需要（用户本侧验收通过，2026-09-03）；如未来复发可临时切换为 `s2idle`。
  ...
  `hwinfo_g0m.bin` 时亮度调节不可用但点亮正常；红屏候选修复未包含且仍未验证；发布许可门禁仍为
  对方项目未正式发布，交付形式为本地 deb + 文档（无 tag/Release）；换用新设备必须重新执行图形基线
  和至少一次受控挂起验收。dotfiles 侧无可修项。
