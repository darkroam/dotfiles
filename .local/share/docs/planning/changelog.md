...
- [x] 2026-07-31：新增 `rebuild-suckless` 和 `cfg-check`。前者提供统一的 suckless 四仓库
  编译安装入口，支持按组件或全量构建，按 dmenu → dwmblocks → st → dwm 依赖顺序执行。后者
  验证仓库健康状态：已跟踪文件存在、`.local/bin/` 脚本权限和 shebang、文档内部链接完整、
  无 `PRIVATE KEY` 泄漏、未跟踪文件提示。两项均为 POSIX Shell，已通过自身验证。架构文档
  同步更新。
...
- [x] 统一 Transmission daemon 所有权：共享脚本只连接平台服务管理器持有的 RPC，
  `td-toggle` 改为暂停或恢复全部任务；Debian 平台已验证服务和 loopback-only RPC，真实任务交互
  仍由平台待办跟踪。
