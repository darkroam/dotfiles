...
| `.config/mimeapps.list`、`.local/share/applications/{file,img,mail,pdf,rss,text,torrent,video}.desktop` | `st`、`lfub`、`nsxiv`、`neomutt`、`zathura`、`rssadd`、`transadd`、`nvim`、`mpv`、WPS Office 桌面文件 | MIME 默认程序和自定义桌面入口 | 均已安装或为已跟踪本地脚本；`wps-office-prometheus.desktop` 存在于 `/usr/share/applications` | 文件、文档与桌面处理 | 待核对 | 所有 `Exec` 路径可解析；实际桌面启动需在 X11 会话复查 |
