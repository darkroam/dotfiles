...
| `.local/bin/displayselect` | `xrandr`、`arandr`、`flock` | 显示布局选择、手动布局入口和共享布局锁 | 已安装；Debian 包分别为 `x11-xserver-utils`、`arandr`、`util-linux` | 显示、网络、挂载与系统控制 | 待核对 | 已在真实双屏 X11 会话验证精确输出筛选和主屏设置；脚本会对缺失的依赖提示安装，并与 watcher 串行修改布局 |
| `.local/bin/remaps`、`.local/bin/sysact`、`.local/bin/setbg`、`.local/bin/xdisplay.sh` | `xrandr`、`flock`、`xset`、`xdotool`、`dunst`、`bc`、`xcape`、`slock`、`xwallpaper`、`wal` | 显示管理、键盘重映射、锁屏、壁纸与可选配色 | 均已安装；`wal` 为本地可执行文件 | 显示、网络、挂载与系统控制；外观、字体与壁纸 | 待核对 | Shell 语法检查通过；新版 `xdisplay.sh` 使用单快照、动态轮询、结果验证、单实例/布局锁和锁内有界内屏恢复；尚待真实开盖、合盖、插拔、慢速外屏接入和镜像回退复查；壁纸和锁屏也待图形会话复查 |
