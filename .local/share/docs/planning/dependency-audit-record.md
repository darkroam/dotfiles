...
状态：Debian 首轮审计已完成。发现缺失依赖时，仅暂停对应项目；不得以未验证的替代项
自动改写配置。`progs.csv` 的历史迁移已结束，后续发行版审计不再处理该文件。
...
| `README.md`、`.local/share/docs/**`、`.local/share/larbs/progs.csv`、`.local/bin/cron/README.md` | 无直接运行时依赖；内容包含项目依赖说明 | 项目说明、计划与历史数据 | 不适用 | 文档与规划 | 已完成 | 首轮已完成 `progs.csv` 迁移、依赖清单完整性和用户/架构文档对齐；后续仅按新的审计流程维护 |
