...
- [x] 2026-07-17：修正 DWM `Mod+F1` 帮助 PDF 的命令和快捷键标签显示。用户已在 Zathura
  实测嵌入字体后字符显示正确；最终采用 `fonts-urw-base35` 提供的常规字重 Nimbus Sans，避免
  查看器替代 Courier 时字符挤压或重叠，并保持七页内容和现有快捷键不变。维护策略和跨发行版
  审计流程已固化生成类文档检查：
...
- [x] 2026-07-16：修复 innogpu 拔屏后 `disconnected + geometry` 残留和扩展坞无 preferred 时
  ...
  与 `11/11` 显示回归。真实会话完成“扩展坞拔出、接回、合盖、开盖、再次拔出”链路，外屏自动
  收敛到 `2560x1440@59.95`，断开输出无 geometry，framebuffer 全程匹配有效输出。

- [x] `0b40ac7`：完成显示状态解析和 watcher 生命周期重构；2026-07-16 整机重启验证新 X11
  会话可以取得 watch lock。另一块外屏完成预接冷启动测试和开合盖验证，但原生模式未自动收敛，
  不能记为通过，也不替代原问题外屏复测。一次合盖慢恢复 B 组结果初步支持 framebuffer 重建后
  的驱动或输出链路重新同步这一候选原因；继续采用通用 framebuffer 收敛，保留旧尺寸只作为
  受控诊断分支。
