...
- [ ] 按[显示管理重构计划](display-management-redesign.md)完成阶段 3：实现单一未跟踪设备适配器，
  迁移本机内屏候选和恢复钩子，并验证标准设备零配置路径。
- [ ] 完成显示阶段 4 的剩余项目：统一 desired/off 规划器、与 manual marker 协调的完整布局健康、
  有盖/无盖设备策略、最高共同镜像模式，以及冷启动、多个外屏、桌面设备和手动布局矩阵。
