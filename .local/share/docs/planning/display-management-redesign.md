...
状态：方案与文档复核通过，尚未改变当前显示行为；下一步进入阶段 1。代码回退基线为提交
`a191c3c`。
...
- [ ] 创建隔离目录、`RESTORE.md` 和 sha256 清单，先复制活动系统配置作为快照。
- [ ] 为隔离目录加入精确忽略规则，确认新建的恢复资料不会进入配置仓库。
- [ ] 先移走无效 udev 规则并 reload/验证，再移动其 RUN 指向的旧 hybrid 脚本；随后移动无引用的
  `xinit` 和核对过的 Xorg 历史备份。
- [ ] 确认没有第二条自动执行 `xrandr` 的链路；现有 watcher 行为保持不变。
- [ ] 完成一次当前版本的开盖、合盖、热插和热拔基线验证。
...
阶段 1 只能在以下条件同时满足后开始：本计划与设备适配器契约已确认；所有文档链接、术语、
