...
只有设备无法按标准输出名识别内屏，或驱动在开盖后不能自行恢复内屏模式时，才创建本机适配器：

```text
~/.config/x11/xdisplay-device.local
```

该文件是未跟踪的本机可执行文件，是显示管理唯一预期的设备个性化入口。它只回答“哪些输出可能是
...
驱动模块、Xorg 配置和 logind 电源策略属于系统层，不得复制进适配器。确有系统级配置时，应另行
记录来源和恢复方法，不能以适配器已经存在为由假设这些配置在新设备上也存在。
...
- 没有额外候选时输出为空并返回 `0`；参数或本机配置错误时返回非零。
...
输出名必须来自本机当前 `xrandr --query`，大小写和连字符完全一致。不要因为 DRM connector 名
相似就直接假定 RandR 名称相同。
...
探测记录不得包含用户名、主机名、序列号、EDID 原始数据或其他个人信息。需要进入项目文档的结论
只记录通用行为和必要的非敏感设备边界。

## 适配器示例

以下示例假定某设备把内屏暴露为非标准的 `DP-1`。它只展示接口结构；模式恢复部分默认无操作，
必须先按上一节取得并验证本机参数后才能补充。
...
~/.config/x11/xdisplay-device.local internal-outputs
~/.config/x11/xdisplay-device.local restore-internal DP-1
```

不要把示例中的 `DP-1` 当作通用默认值。外屏始终由运行时连接状态发现，不应出现在候选列表或
...
接电合盖后继续使用外屏的设备，需要在系统层明确评估 `HandleLidSwitchExternalPower=ignore`；
使用电池、docked 状态以及发行版默认值仍可能采用不同策略。修改 `/etc/systemd/logind.conf` 前应先
确认预期的电源行为，并记录原值和恢复方式。通用引擎不得为了显示布局自动改写 logind 配置，
设备适配器也不得调用 `loginctl`、`systemctl` 或模拟盖子事件。
...
6. 在迁移验证完成前保留旧本机恢复脚本和过渡文件的隔离备份。需要整体回退时，按显示管理重构计划
   中记录的基线提交、原路径、权限和恢复步骤执行，不要同时启用新旧 watcher。

只有验证矩阵持续通过，旧恢复钩子才可以进入待清理隔离目录；隔离目录的最终删除仍属于挂起任务，
不能由适配器安装过程自动完成。
