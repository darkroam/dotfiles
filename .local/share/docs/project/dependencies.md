...
| Bash 补全包和 Git 补全脚本 | `c` 和 Git 的 Bash 补全 |
...
| ALSA 库和工具 | 保留的 ALSA 配置和 ALSA 捕获输入 |
...
`tutorialvids`、`remaps` 和 `zzz`。其外部要求列于上方相关章节。
...
| `youtube-viewer` | 最低优先级可选 Newsboat 视频宏；常规 RSS 和视频处理使用已有浏览器、`mpv`、`yt-dlp` 与 `linkhandler`，因此排除在 Debian 审计外 |
...
| Facebook PathPicker (`fpp`) | 最低优先级可选 tmux 路径选择绑定；排除在 Debian 审计外，正常部署不需要 |
| `tremc` | 最低优先级可选 Transmission 终端界面。没有它时 `torrent` 启动守护进程并引导用户使用 Web 界面或 `transmission-remote`；排除在 Debian 审计外 |
...
`DBUS_SESSION_BUS_ADDRESS`，随后加载 `.profile`。`checkup` 使用 `sudo apt update`；
无人值守使用需要范围严格的免密 sudo 规则或手动检查。

## 本机验证记录

这不是部署要求。当前 Debian 验证状态维护在 `planning/dependency-audit-record.md`；
不得将旧软件包快照视为当前需求清单。
...
| `libX11-devel`, `libXft-devel`, `gcr-devel`, `fontconfig-devel` | Build dependencies | 为单独构建程序保留的历史 Arch/Void 开发包名称；不是当前 dotfiles 运行依赖。 |
...
| 来源记录 | 原始用途 | 当前处置 |
| --- | --- | --- |
| `gtk-theme-arc-gruvbox-git` | Dark GTK theme | 历史 Arch 包名；已跟踪外观使用已安装 Arc 主题。 |
...
| `ntfs-3g` | NTFS access | 可选文件系统支持；无已跟踪辅助命令直接调用，当前内核也可能提供 NTFS3。 |
