...
DWM `Mod+F1` 使用 Groff 将 `/usr/local/share/dwm/larbs.mom` 动态生成 PDF 并交给 Zathura；
帮助源码由 `~/src/dwm/larbs.mom` 安装，命令和快捷键标签使用嵌入式 Nimbus Sans 字体，避免
PDF 查看器替代字体造成字宽错配。
...
`dmenumount` 和 `dmenuumount` 只处理 `lsblk` 可见的普通块设备，CIFS 由独立命令负责；两类
挂载入口不得互相混入协议专用分支。
...
仓库跟踪的启动入口仍是 `xprofile`，布局已满足时不得重复 modeset。
