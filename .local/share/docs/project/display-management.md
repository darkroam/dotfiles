...
> 服务域：U06 显示、网络、挂载与系统控制；关联 U02 X11 桌面与输入、U03 外观、字体与壁纸、S02 验证

本文面向维护者，是 display 子系统的共享 X11 显示引擎权威设计文档，负责命令结构、状态、配置、
默认显示布局、自定义显示布局、锁、重试和当前/目标边界。非标准硬件扩展接口见
...
- 可选引擎参数与默认显示布局配置，非法项逐项诊断并回退；
- 自定义显示布局的保存、精确/包含匹配、恢复和删除；
...
设备的独立多屏扩展回退、有效 manual marker 与完整自动显示布局 health，以及可选显式 `--fb` 收敛。
保留的 `MIRROR`、`CUSTOM` 状态常量不是当前对外状态；镜像是回退显示布局，自定义配置命中后仍保留
物理/lid 状态名。
...
1. **引擎管策略，适配器管设备差异。** 通用引擎统一负责状态、显示布局、锁、验证和重试；适配器只
...
```text
X11 会话 -> xprofile -> xdisplay watch -> 状态计算 -> 默认或自定义显示布局
Mod+F3 ----------------> displayselect -> 交互显示布局或 Arandr
```
...
| `.local/bin/displayselect` | 交互选屏及 `save`、`list`、`delete` 自定义显示布局命令 |
| `.local/lib/xdisplay/` | 状态、配置、显示布局、适配器、自定义显示布局、日志和 watcher 的共享实现 |
| `.config/x11/xprofile` | 在正确的 X11 会话环境中启动唯一 `xdisplay watch` |

所有共享库函数使用 `xdisplay_` 前缀。自动和手动入口共用 `apply.lock`；每个规范化 X server 只有
一个 `watch.lock`。不得同时增加 udev 直接 `xrandr`、第二个 watcher 或缺少图形会话环境的显示布局服务。
...
补充模式会触发重算。拓扑签名包含 lid 状态、输出连接、首项模式、能力签名和自定义显示布局名称/mtime。
...
合盖时内屏仍保留在诊断列表，但不计入有效状态。自定义显示布局命中时 `layout=custom`、`custom=NAME`，
状态名仍是上表的实际状态。
...
默认显示布局：`~/.config/x11/display-layouts/default.conf`
...
`mirror_on_duplicate` 当前只解析和显示，不改变显示布局。
...
每次写入后都重读 RandR，验证 active、primary、原点、目标模式和链式方向；自定义显示布局还验证保存的
...
诊断、跳过该候选并使用默认显示布局；RandR 应用或收敛验证失败时返回 watcher 的有界重试路径，
多输出路径在应用成功但重读未收敛时还会清除本次命中并尝试默认显示布局。任何一种失败都不能阻塞 watcher
主循环。
...
期望 active/off、primary、geometry、target mode 和 framebuffer 的完整自动显示布局 health。

`displayselect` 取得同一 apply lock，支持单屏、双屏选择、有限三屏交互和可选 Arandr；自定义显示布局
管理也复用共享库。generation、manual marker 路径及只读状态已经存在，但当前没有 `--manual-run`，
也不会写有效 marker，所以 watcher 尚不能长期保留一个拓扑未变的任意手动显示布局。DWM 会在根窗口
尺寸变化后通过 ConfigureNotify/Xinerama 重新读取几何，正常 RandR 切换不要求重启 DWM。

## 状态、日志与故障排查

`xdisplay status` 只读，不恢复模式或写显示布局。它输出 lid、物理状态枚举、当前显示布局函数、
自定义配置、
配置摘要、framebuffer、每个输出的模式/几何/stale/pending、policy、health、拓扑签名、锁、generation、
manual marker 和 legacy 配置可用性。

适配器和自定义显示布局诊断默认写入 `~/.local/share/x11/xdisplay-adapter.log`，或写入配置的
`log_path`。文件以 `0600` 创建，达到 `log_max_bytes` 后覆盖轮转为 `.1`；日志失败不改变显示布局
返回值。详细字段和
...
共享目标是 framebuffer 与自动显示布局的有效输出包围盒一致。普通 `--output ... --off` 能让 Xorg
自动收敛时不得增加显式 `--fb`。只有统一 desired/off 规划已完成且显示布局重读正确、framebuffer
仍稳定
残留时，才能另行设计可关闭的显式路径。

实现前必须校验 RandR minimum/maximum、计划包围盒、坐标和 panning；不得改写合法的手动负坐标、
缩放或排列。诊断 A/B 只允许在 apply lock 内动态取得输出、模式和旧 framebuffer，设置总超时与
恢复 trap，测试后恢复标准显示布局。设备实测耗时和结果只写平台档案。
...
| exact/contains 自定义配置 | 恢复自定义显示布局快照；contains 的多出输出按默认链追加 |
| 配置解析损坏 | 记录诊断、跳过候选并回退默认显示布局 |
| 自定义显示布局应用或验证失败 | 返回非零并进入 watcher 有界重试；不得停止主循环或关闭全部输出 |
...
- 扩展 `displayselect` 的任意数量交互显示布局；当前任意数量能力属于自动链和自定义显示布局；
...
- 状态由只读快照计算，显示布局函数不能反向修改状态事实。
- 自动、手动、自定义显示布局和适配器恢复共享同一写锁；适配器不得取得显示布局所有权。
- 先保留一个已验证活屏，再关闭其他输出；任何路径不得制造“所有输出都关闭”的瞬间。
- 配置缺失或损坏、适配器失败、自定义显示布局失效时必须降级，不阻塞 X11 会话。
