...
| `~/src/dwm/config.h` | `abook`、`profanity`、`wpctl`、本地 `passmenu`、`zzz` | 通讯录、XMPP、麦克风静音、密码菜单和睡眠键的 DWM 绑定 | `abook`、`profanity` 已安装；`wpctl` 已由 Debian `wireplumber` 包提供；`passmenu` 已跟踪于 `~/.local/bin`；`zzz` 当前不在 PATH，尚无本平台提供者结论 | 文件、文档、密码与桌面处理；状态栏、通信与网络服务；音频、音乐、录制与视频；显示、网络、挂载与系统控制 | 不适用（独立源码仓库） | DWM `Mod+Shift+e` 直接启动 `abook`，`Mod+c` 直接在终端启动 `profanity`，麦克风静音使用 WirePlumber `wpctl`，`Mod+Shift+d` 已有可移植的跟踪入口；只剩 `zzz` 缺失会使对应睡眠键启动失败 |
...
- [ ] 决定如何处理 DWM 睡眠键缺失的 `zzz`：改用 `sysact` 等现有能力、部署平台命令
  或移除绑定；未决定前保持
  源码不变。
