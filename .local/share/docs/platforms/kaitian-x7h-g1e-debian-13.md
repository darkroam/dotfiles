...
| `~/src/{dwm,dwmblocks,dmenu,st}` | C 编译器、`make`、`pkg-config`、`tic` 及 X11/Xft/Xrender/Xinerama/Fontconfig/FreeType/HarfBuzz/X11-XCB/XCB/XCB Res 开发库 | 构建 X11 桌面四个独立源码仓库 | 均已安装；Debian 映射见下方关键映射 | X11 桌面与输入 | 不适用（独立源码仓库） | 四个 origin 均为 `https://github.com/darkroam/<repo>`；当前审计 HEAD 依次为 DWM `a00b789`、DWMBlocks `be39197`、dmenu `9d6f1c4`、st `1f10742`。2026-07-18 在当时基线的隔离副本中逐一 `make clean && make`，四个可执行文件均构建成功；DWM 删除旧睡眠键绑定后再次构建通过。2026-07-19 又从干净状态构建当前 DWMBlocks 和 st，二者均成功；st 的 Groff 手册解析和禁用 GLX 的隔离 Xvfb 右键复制后再次右键粘贴流程通过。应用鼠标上报分支完成源码事件顺序复审，st 安装副本与真实 X11 交互仍待复查 |
...
- [ ] 用户编译安装 st `1f10742` 后，在真实 X11 中复查中文和多行选区、右键
  `CLIPBOARD` 复制/粘贴、中键 `PRIMARY`，以及 Nvim/Tmux 鼠标上报下的普通右键和
  `Shift+左键拖动`/`Shift+右键`本地覆盖；隔离 Xvfb 的 ASCII 核心流程已经通过。
