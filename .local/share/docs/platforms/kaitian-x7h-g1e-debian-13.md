...
| `~/src/{dwm,dwmblocks,dmenu,st}` | C 编译器、`make`、`pkg-config`、`tic` 及 X11/Xft/Xrender/Xinerama/Fontconfig/FreeType/HarfBuzz/X11-XCB/XCB/XCB Res 开发库 | 构建 X11 桌面四个独立源码仓库 | 均已安装；Debian 映射见下方关键映射 | X11 桌面与输入 | 不适用（独立源码仓库） | 四个 origin 均为 `https://github.com/darkroam/<repo>`；审计 HEAD 依次为 DWM `9e2c621`、DWMBlocks `4874d4d`、dmenu `9d6f1c4`、st `d93faf0`。2026-07-18 在隔离副本中逐一 `make clean && make`，四个可执行文件均构建成功，未修改源码仓库 |
...
| `~/src/dwm/config.h` | `abook`、`profanity`、`pactl`、`passmenu`、`zzz` | 通讯录、XMPP、麦克风静音、密码菜单和睡眠键的可选 DWM 绑定 | `abook` 已安装；`profanity`、`pactl`、`passmenu`、`zzz` 当前不在 PATH。Debian 包可提供 `profanity` 和 `pactl`；已安装的 `pass` 只在文档目录提供 `passmenu` 示例；`zzz` 尚无本平台提供者结论 | 文件、文档、密码与桌面处理；状态栏、通信与网络服务；音频、音乐、录制与视频；显示、网络、挂载与系统控制 | 不适用（独立源码仓库） | DWM `Mod+Shift+e` 已改为直接启动 `abook`，与 Mutt Wizard 共用 `~/.abook` 默认数据；通讯录是未跟踪的私人运行数据。其余缺失只使对应快捷键启动失败 |
...
- [ ] 分别决定如何处理当前缺失的 DWM 可选依赖：`profanity`、`pactl`、`passmenu` 和
