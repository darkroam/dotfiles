...
| `~/src/{dwm,dwmblocks,dmenu,st}` | C 编译器、`make`、`pkg-config`、`tic` 及 X11/Xft/Xrender/Xinerama/Fontconfig/FreeType/HarfBuzz/X11-XCB/XCB/XCB Res 开发库 | 构建 X11 桌面四个独立源码仓库 | 均已安装；Debian 映射见下方关键映射 | X11 桌面与输入 | 不适用（独立源码仓库） | 四个 origin 均为 `https://github.com/darkroam/<repo>`；当前审计 HEAD 依次为 DWM `a00b789`、DWMBlocks `be39197`、dmenu `9d6f1c4`、st `1f10742`。2026-07-18 在当时基线的隔离副本中逐一 `make clean && make`，四个可执行文件均构建成功；DWM 删除旧睡眠键绑定后再次构建通过。2026-07-19 又从干净状态构建当前 DWMBlocks 和 st，二者均成功；st 的 Groff 手册解析和禁用 GLX 的隔离 Xvfb 右键复制后再次右键粘贴流程通过，应用鼠标上报分支完成源码事件顺序复审。`/usr/local/bin/st` 与当前源码重建产物的 SHA256 一致；用户在真实 X11 中确认有选区时右键复制、无选区时右键粘贴的核心流程可用，本项关闭 |
