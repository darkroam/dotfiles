...
| 审计基线 | 配置仓库 `416c925`；DWM 仓库 `7006616`；首轮记录 2026-07-17，内容复核 2026-07-18 |
...
| `~/src/{dwm,dwmblocks,dmenu,st}` | C 编译器、`make`、`pkg-config`、`tic` 及 X11/Xft/Xrender/Xinerama/Fontconfig/FreeType/HarfBuzz/X11-XCB/XCB/XCB Res 开发库 | 构建 X11 桌面四个独立源码仓库 | 均已安装；Debian 映射见下方关键映射 | X11 桌面与输入 | 不适用（独立源码仓库） | 四个 origin 均为 `https://github.com/darkroam/<repo>`；审计 HEAD 依次为 DWM `916ec34`、DWMBlocks `4874d4d`、dmenu `9d6f1c4`、st `d93faf0`。2026-07-18 在隔离副本中逐一 `make clean && make`，四个可执行文件均构建成功，未修改源码仓库 |
...
| `~/src/dwm/config.h` | `abook`、`profanity`、`pactl`、`passmenu`、`zzz` | 通讯录、XMPP、麦克风静音、密码菜单和睡眠键的可选 DWM 绑定 | 五个命令当前均不在 PATH；Debian 包分别可提供前三项。已安装的 `pass` 只在文档目录提供 `passmenu` 示例；`zzz` 尚无本平台提供者结论 | 文件、文档、密码与桌面处理；状态栏、通信与网络服务；音频、音乐、录制与视频；显示、网络、挂载与系统控制 | 不适用（独立源码仓库） | 源码引用已确认；缺失只使对应快捷键启动失败。2026-07-18 状态栏源码编辑绑定、规范书签、安装二进制和 renew 后运行映像均已对齐 `~/src/dwmblocks/config.h` |
...
| `.local/bin/statusbar/sb-internet` | `nmtui` | 状态栏网络模块的交互式网络管理界面 | 已安装；Debian 软件包 `network-manager` 提供 | 状态栏、通信与网络服务 | 待核对 | 已明确保留该功能；脚本语法检查通过，实际网络管理需在正常用户会话复查 |
...
| `.local/bin/maimpick`、`.local/bin/otp` | `maim` | 截图、截图式 OTP 二维码导入 | 已安装；Debian 包为 `maim` | 文件、文档、密码与桌面处理 | 待核对 | 可执行；关联脚本语法检查通过，实际 X11 截图需在图形会话复查 |
| `.local/bin/otp` | `zbarimg` | OTP 二维码内容识别 | 已安装；Debian 包 `zbar-tools` 提供 | 文件、文档、密码与桌面处理 | 待核对 | 可执行；关联脚本语法检查通过 |
| `.local/bin/otp` | `pass-otp` | 基于 `pass` 的一次性密码管理 | 已安装；Debian 包为 `pass-extension-otp` | 文件、文档、密码与桌面处理 | 待核对 | `pass otp --help` 通过；实际密码库操作不在本轮执行 |
| `.local/bin/otp` | `timedatectl` 或 `chronyc` 或 `ntpdate` | OTP 时钟同步 | `timedatectl` 已安装；其运行状态受沙箱 systemd 总线限制，未验证 | 文件、文档、密码与桌面处理 | 待核对 | 脚本已有按顺序回退逻辑；本轮不要求安装额外时间同步客户端 |
...
| `.local/bin/remaps`、`.local/bin/sysact`、`.local/bin/setbg`、`.local/bin/xdisplay.sh` | `xrandr`、`flock`、`xset`、`xdotool`、`dunst`、`bc`、`xcape`、`slock`、`xwallpaper`、`wal` | 显示管理、键盘重映射、锁屏、壁纸与可选配色 | 均已安装；`wal` 为本地可执行文件 | 显示、网络、挂载与系统控制；外观、字体与壁纸 | 主要流程已完成 | Shell 语法检查通过；本机已验证 `xdisplay.sh` 的开合盖、热插入、拔出、动态轮询、合盖快速路径和单实例/布局锁。原审计发现的 innogpu 拔屏后 disconnected geometry/framebuffer 残留，后续已通过显式关闭、重读验证和完整实机链路修复；依赖集合未变化。壁纸和锁屏仍按各自图形会话路径维护 |
...
| `.config/mimeapps.list`、`.local/share/applications/{file,img,mail,pdf,rss,text,torrent,video}.desktop` | `st`、`lfub`、`nsxiv`、`neomutt`、`zathura`、`rssadd`、`transadd`、`nvim`、`mpv`、WPS Office、`clash-verge` 桌面文件 | MIME 默认程序、自定义桌面入口和 Clash URI 方案处理器 | 自定义入口及 WPS 可解析；系统实际文件为 `Clash Verge.desktop`，当前未提交工作树却引用不存在的 `clash-verge.desktop` | 文件、文档、密码与桌面处理 | 待核对 | 已跟踪基线使用系统实际 desktop ID；当前 Clash URI 关联失效并列入平台待办，本次不修改或提交该并发改动 |
...
| OTP | `pass-extension-otp`、`zbar-tools`；时钟检查当前由 `timedatectl` 满足 |
| CIFS | `cifs-utils`、`smbclient`、`avahi-utils`；`mount.cifs` 位于 `/usr/sbin` |
...
- [ ] 复查仍依赖真实 X11、硬件、账户或网络的交互路径：NetworkManager、截图/录制、OTP、CIFS、
  Transmission、RSS/邮件和媒体预览。
- [ ] 分别决定如何处理当前缺失的 DWM 可选依赖：`abook`、`profanity`、`pactl`、`passmenu` 和
  `zzz`。可选择安装、部署示例、改用 `wpctl`/`sysact` 等现有能力或移除绑定；未决定前保持
  源码不变。
- [ ] 按“运行必要性、跨设备价值、效率、结构和维护成本”评估未跟踪的 HDA audio user unit/helper；
  未经单独审查不纳入配置仓库。
- [ ] 决定 Clash URI 应继续使用系统 `Clash Verge.desktop`，还是采用并跟踪单独 handler；当前未提交
  `mimeapps.list` 改动引用不存在的 `clash-verge.desktop`，在决定前不得纳入提交。
