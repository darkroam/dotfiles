...
| `.config/shell/zprofile` | `zsh`、`startx`、Xorg、`fbterm`、`tty`、`pgrep` | 必需（登录、本地 X11 启动和恢复 TTY 时） | 已安装；`startx` 由 `xinit`、Xorg 由 `xserver-xorg-core` 提供；fbterm 使用本机已验证的 redraw 版本 | Shell、源代码管理与开发；X11 桌面与输入 | 待核对 | `zsh -n` 通过；登录入口不再执行 `sudo -n loadkeys`，避免无 sudo 缓存时生成 `mail_badpass` 邮件；`ttymaps.kmap` 仅保留为可选手动资源；TTY1 保持 X11 启动入口，TTY2-9 在非 SSH、非图形登录时自动进入 fbterm，退出或启动失败后保留普通登录 Shell；图形门槛动态接受任意 `/dev/dri/cardN` 或 `/dev/fbN` |
...
| `.local/bin/maimpick` | `maim`；菜单另需 `dmenu`，窗口另需 `xdotool`，复制另需 `xclip`；GNU coreutils；可选 `notify-send` | 选区、当前窗口和全屏的唯一保存或图片剪贴板入口 | 均已安装；Debian 包为 `maim`、`xdotool`、`xclip`、`coreutils`、`libnotify-bin`，`dmenu` 由本机源码构建 | 音频、音乐、录制与视频 | 不存在 | Dash、系统 sh、BusyBox sh 语法通过；隔离 mock `33/33` 和不覆盖补充测试 `4/4` 通过。真实 X11 中，`Print` 在默认目录生成 mode 600 的有效 `2560x1440` PNG；六项菜单、选区与当前窗口复制均可用，窗口 PNG 为 `2500x1396` 且透明区域下方画面正确；`Escape` 取消选区后文本 sentinel 保持不变；绝对 `MAIMPICK_DIR` 在临时目录生成 mode 600 的有效 `2560x1440` PNG，测试目录已清理 |
