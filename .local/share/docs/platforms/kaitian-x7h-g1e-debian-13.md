...
| `~/src/dwm/config.h` | `abook`、`profanity`、`wpctl`、本地 `passmenu` | 通讯录、XMPP、麦克风静音和密码菜单的 DWM 绑定 | `abook`、`profanity` 已安装；`wpctl` 已由 Debian `wireplumber` 包提供；`passmenu` 已跟踪于 `~/.local/bin` | 文件、文档、密码与桌面处理；状态栏、通信与网络服务；音频、音乐、录制与视频 | 不适用（独立源码仓库） | DWM 保留已验证的应用键，已删除 `XF86XK_Sleep`；硬件睡眠键交由 logind 唯一处理，不再存在 `zzz` 运行依赖 |
...
| `.local/bin/sysact` | `dmenu`、systemd `systemctl` 或 elogind `loginctl`、`slock`、`wpctl`、`mpc`、本地 `pauseallmpv`、`pstree`、`xset`、`notify-send` | 交互式及显式的锁屏、会话、电源和显示操作 | 均已安装或为已跟踪本地命令；Debian 包 `systemd` 提供 `systemctl`，`psmisc` 提供 `pstree`，`suckless-tools` 提供 `slock`，其余提供者已在所属批次审计 | 显示、网络、挂载与系统控制；音频、音乐、录制与视频 | 不存在 | `dash -n` 通过；无参数菜单与单参数动作共用分派，`systemctl` 动作不再使用 `-i`；stub 回归不调用真实锁屏、电源或会话命令，真实挂起/休眠待实机验证 |
...
- [ ] 重启 X11 会话并安装新 DWM 后，确认 `xss-lock` 只有一个实例，硬件睡眠键
  只触发一次挂起，恢复时仍需 `slock` 认证；另用非破坏方式确认 `sysact`
  的 inhibitor 拒绝不会降级到旧电源命令。本轮不实际执行休眠。
