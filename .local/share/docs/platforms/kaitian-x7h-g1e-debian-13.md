...
| `~/src/{dwm,dwmblocks,dmenu,st}` | C 编译器、`make`、`pkg-config`、`tic` 及 X11/Xft/Xrender/Xinerama/Fontconfig/FreeType/HarfBuzz/X11-XCB/XCB/XCB Res 开发库 | 构建 X11 桌面四个独立源码仓库 | 均已安装；Debian 映射见下方关键映射 | X11 桌面与输入 | 不适用（独立源码仓库） | 四个 origin 均为 `https://github.com/darkroam/<repo>`；当前审计 HEAD 依次为 DWM `bb19a76`、DWMBlocks `be39197`、dmenu `9d6f1c4`、st `1f10742`。2026-07-18 在当时基线的隔离副本中逐一 `make clean && make`，四个可执行文件均构建成功；2026-07-19 又从干净状态构建当前 DWMBlocks、st 及功能提交 `38f3041` 的 DWM，三者均成功；2026-07-20 在帮助提交 `bb19a76` 上再次干净构建 DWM 成功。st 的隔离右键流程和安装后实测已经完成；DWM `bb19a76` 尚未安装和 renew |
| `~/src/dwm/{config.h,larbs.mom}` | `groff`、`fonts-urw-base35`、`zathura`、Zathura PDF 后端 | `Mod+F1` 动态帮助 | 已安装；当前 PDF 后端由 Debian `zathura-pdf-poppler` 提供 | X11 桌面与输入；编译、排版与数据辅助 | 不适用（独立源码仓库） | 新源码帮助生成 7 页非空 PDF，文本包含全屏截图和六项菜单，`NimbusSans-Regular` 为 `emb=yes`，第 5 页渲染无重叠或裁切；`/usr/local/share/dwm/larbs.mom` 仍是安装前旧副本，待用户安装 DWM 后复核一致性和实际 `Mod+F1` |
...
| `.local/bin/maimpick` | `maim`；菜单另需 `dmenu`，窗口另需 `xdotool`，复制另需 `xclip`；GNU coreutils；可选 `notify-send` | 选区、当前窗口和全屏的唯一保存或图片剪贴板入口 | 均已安装；Debian 包为 `maim`、`xdotool`、`xclip`、`coreutils`、`libnotify-bin`，`dmenu` 由本机源码构建 | 音频、音乐、录制与视频 | 待核对 | Dash、系统 sh、BusyBox sh 语法通过；隔离 mock `33/33` 覆盖六项菜单、按分支依赖、取消/错误、剪贴板保持、空输出、临时清理、默认/覆盖目录、并发唯一名以及 GNU/BusyBox core 路径。真实 X11 和新 DWM 绑定仍待安装后复查 |
| `.local/bin/otp` | `maim` | OTP 二维码区域选择 | 已安装；Debian 包为 `maim` | 文件、文档、密码与桌面处理 | 待核对 | OTP 不创建图片文件，`maim` 的 PNG stdout 直接送入识别器；实际 X11 区域选择仍需会话复查 |
...
- [ ] 用户安装 DWM `bb19a76` 并 renew 后，在真实 X11 复查 `Print`、六项 `Shift+Print` 菜单、
  选区取消保持剪贴板、透明窗口 `-B`、图片粘贴、默认 `~/Pictures` 和自定义 `MAIMPICK_DIR`；
  同时确认安装帮助副本与源码一致，并用 `Mod+F1` 检查 Zathura 第 5 页。
