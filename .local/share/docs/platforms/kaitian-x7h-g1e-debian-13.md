...
| `.config/shell/zprofile` | `zsh`、`sudo`、`loadkeys`、`startx`、Xorg、`fbterm`、`tty`、`pgrep`、`ttymaps.kmap` | 必需（登录、本地 X11 启动和恢复 TTY 时） | 已安装；`loadkeys` 由 `kbd`、`startx` 由 `xinit`、Xorg 由 `xserver-xorg-core` 提供；键盘映射可读；fbterm 使用本机已验证的 redraw 版本 | Shell、源代码管理与开发；X11 桌面与输入 | 待核对 | `zsh -n` 通过；TTY1 保持 X11 启动入口，TTY2-9 在非 SSH、非图形登录时自动进入 fbterm，退出或启动失败后保留普通登录 Shell；图形门槛动态接受任意 `/dev/dri/cardN` 或 `/dev/fbN` |
