...
显示 lid、各输出的连接/geometry/mode、stale/pending、当前策略、锁路径、watcher generation
和 manual marker 状态。内屏名称为标准
...
合盖会先使用当前缓存布局快速切换，只有失败或输出未就绪时才主动重新探测硬件。首次布局已经
成功但显示器能力随后才变化时，当前版本可能要等低频探测才再次处理，排查时应同时检查当前模式。
手动显示选择期间自动布局会等待，避免两者互相覆盖。缺少 `xrandr` 或 `flock` 时脚本会提示安装；
在显示选择器中选择手动布局但缺少 `arandr` 时也会提示安装。

本机 innogpu 在拔屏后可能仍让已断开的 HDMI 带有旧几何，使物理屏幕已经切换但 X11 framebuffer
没有完全收缩。可用 `xdisplay.sh --status` 检查 `stale_outputs`，或用 `xrandr --current` 检查
`disconnected` 行是否仍含分辨率和坐标；该清理逻辑仍在活动 TODO 中。
